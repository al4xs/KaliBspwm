...
export PATH=~/go/bin:/usr/local/sbin/jadx-1.4.4/bin:/usr/local/sbin/ghidra_10.1.5_PUBLIC:/usr/bin/java:$PATH
...
if [ $IP1 ]; then
	LOCAL="%F{green}─🮤 %F{cyan}$IP1%b%F{green}🮥  %F{cyan}  %*%f %F{green}"
else
	LOCAL=""
fi

if [ $IP2 ]; then
    VPN="%F{green}─🮤 %F{yellow}$IP2%b%F{green}🮥  %F{cyan}  %*%f %F{green}"
else
    VPN=""
fi

if [ $IP3 ]; then
    WIFI="%F{green}─🮤直 %F{red}$IP3%F{green}🮥   %F{cyan}  %*%f %F{green}"
else
    WIFI=""
fi

DIR=$'%B%F{yellow}%(6~.%-1~/…/%4~.%5~)%F{green}'
NAME=$'%F{blue} %F{magenta} Al4xs %F{blue}  '
...
source ~/.gf-completions/gf-completion.zsh
